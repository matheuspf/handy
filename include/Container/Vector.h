...
/** 'cnt::Vector' inherits from 'std::vector<T>' if it is not supplied with compile time size
  * or if the given compile time size is greater than the value defined at 'cnt::impl::cnt::maxSize',
...
{
    using Base = impl::cnt::SelectType<T, N>;
    ...
    /// If compile time size 'N' is greater than 'impl::cnt::maxSize', initialize the 'std::vector' with this size
